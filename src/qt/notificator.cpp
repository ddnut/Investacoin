...
const int BYINV_PER_PIXEL = 4;
const int BITS_PER_SAMPLE = 8;

FreedesktopImage::FreedesktopImage(const QImage &img):
    width(img.width()),
    height(img.height()),
    stride(img.width() * BYINV_PER_PIXEL),
    hasAlpha(true),
    channels(CHANNELS),
    bitsPerSample(BITS_PER_SAMPLE)
{
    ...
    image.resize(num_pixels * BYINV_PER_PIXEL);

    for(unsigned int ptr = 0; ptr < num_pixels; ++ptr)
    {
        image[ptr*BYINV_PER_PIXEL+0] = data[ptr] >> 16; // R
        image[ptr*BYINV_PER_PIXEL+1] = data[ptr] >> 8;  // G
        image[ptr*BYINV_PER_PIXEL+2] = data[ptr];       // B
        image[ptr*BYINV_PER_PIXEL+3] = data[ptr] >> 24; // A
    }
}
