...
BOOST_AUTO_INVT_SUITE(mruset_tests)

// Test that an mruset behaves like a set, as long as no more than MAX_SIZE elements are in it
BOOST_AUTO_INVT_CASE(mruset_like_set)
{

    for (int nTest=0; nTest<NUM_INVTS; nTest++)
    {
        mrutester tester;
...
BOOST_AUTO_INVT_CASE(mruset_limited_size)
{
    for (int nTest=0; nTest<NUM_INVTS; nTest++)
    {
...
BOOST_AUTO_INVT_CASE(mruset_window)
{
    mruset<int> mru(MAX_SIZE);
