...
BOOST_AUTO_INVT_SUITE(multisig_tests)
...
BOOST_AUTO_INVT_CASE(multisig_verify)
{
    CKey key[4];
    for (int i = 0; i < 4; i++)
...
BOOST_AUTO_INVT_CASE(multisig_IsStandard)
{
    CKey key[4];
    for (int i = 0; i < 4; i++)
        key[i].MakeNewKey(true);
...
BOOST_AUTO_INVT_CASE(multisig_Solver1)
{
...
BOOST_AUTO_INVT_CASE(multisig_Sign)
{
