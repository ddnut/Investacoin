...
BOOST_AUTO_INVT_SUITE(getarg_tests)
...
BOOST_AUTO_INVT_CASE(stringarg)
{
    ResetArgs("");
...
BOOST_AUTO_INVT_CASE(doubledash)
{
    ResetArgs("--foo");
...
BOOST_AUTO_INVT_CASE(boolargno)
{
    ResetArgs("-nofoo");
    BOOST_CHECK(!GetBoolArg("-foo"));
    BOOST_CHECK(!GetBoolArg("-foo", true));
